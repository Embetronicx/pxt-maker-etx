...
namespace pxsim.fonts {
    export function charCodeBuffer(charCode: number): RefBuffer {
        if (charCode < DAL.BITMAP_FONT_ASCII_START || charCode > DAL.BITMAP_FONT_ASCII_END)
            return undefined;
        ...
        const h = font.height;
        const w = font.width;
        const buf = pxsim.BufferMethods.createBuffer(h);
        const offset = (charCode - DAL.BITMAP_FONT_ASCII_START) * h;
